 ...
 */
export async function postAction(input: PostActionInput): Promise<PostActionResult> {
	...
	try {
		...
		for (const file of files) {
			...
			if (uploadError) {
				return { ok: false, error: `Upload failed for ${file.name}: ${uploadError.message}` };
			}

			const { data: publicData } = supabase.storage.from(bucket).getPublicUrl(filePath);
			if (!publicData?.publicUrl) {
				return { ok: false, error: `Failed to get public URL for ${file.name}` };
			}
		...
		if (insertError || !insertedPost) {
			return { ok: false, error: insertError?.message || "Failed to insert post" };
		}
		...
		if (tags.length > 0) {
			...
			if (missing.length > 0) {
				...
				if (insertTagsError) {
					...
					const constraintConflict = insertTagsError.message?.toLowerCase().includes("duplicate") || insertTagsError.code === "23505";
			...
			if (tagIds.length > 0) {
				...
				const { error: linkError } = await supabase.from("post_tags").insert(joinRows);
				if (linkError) {
